package cn.skyliuyang.iHadoop.baseStation;
...
public class BaseStationDataPreprocess extends Configured implements Tool {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void setup(Context context) throws IOException {
			...
			this.timepoint = context.getConfiguration().get("timepoint")
					.split("-"); // 读取时间分割点
	...
	public static class Reduce extends Reducer<Text, Text, NullWritable, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			try {
				// 在最后添加“OFF”位置
				Date tmp = this.formatter.parse(this.date + " "
						+ timeFlag.split("-")[1] + ":00:00");
	...
	public static void main(String[] args) throws Exception {
		if (args.length != 4) {
			...
			System.err.println("\t" + "TIMESKIP" + "\t"
					+ "Lines which contain wrong date format");
			System.err.println("\t" + "OUTOFTIMESKIP" + "\t"
					+ "Lines which contain times that out of range");
			System.err.println("\t" + "LINESKIP" + "\t"
					+ "Lines which are invalid");
			System.err.println("\t" + "USERSKIP" + "\t"
					+ "Users in some time are invalid");
			System.exit(-1);
		}

		// 运行任务
		int res = ToolRunner.run(new Configuration(),
				new BaseStationDataPreprocess(), args);
