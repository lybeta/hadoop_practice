package cn.skyliuyang.iHadoop.pagerank;
...
public class PageRankJob {
    ...
    private static Map<String, String> peoplerank() {
        ...
        path.put("input", HDFS + "/user/hadoop/pagerank");// HDFS的目录
        path.put("input_pr", HDFS + "/user/hadoop/pagerank/pr");// pr存储目
        path.put("tmp1", HDFS + "/user/hadoop/pagerank/tmp1");// 临时目录,存放邻接矩阵
        path.put("tmp2", HDFS + "/user/hadoop/pagerank/tmp2");// 临时目录,计算到得PR,覆盖input_pr

        path.put("result", HDFS + "/user/hadoop/pagerank/result");// 计算结果的PR
        return path;
